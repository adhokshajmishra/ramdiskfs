...
#define BV_LEN(bv) ((bv).bv_len)
	...
	rq_for_each_segment(bv, req, iter)
	{
		...
		printk(KERN_DEBUG "rb: Start Sector: %lld, Sector Offset: %lld; Buffer: %p; Length: %u sectors\n",
...
static int __init rb_init(void)
{
	...
	/* Get Registered */
	rb_major = register_blkdev(rb_major, "amishra");
	if (rb_major <= 0)
	{
	...
	if (rb_dev.rb_queue == NULL)
	{
		...
		unregister_blkdev(rb_major, "amishra");
		ramdiskfs_cleanup();
		return -ENOMEM;
	}
	...
	sprintf(rb_dev.rb_disk->disk_name, "amishra");
	...
	printk(KERN_INFO "rb: ramdiskfs driver initialised (%d sectors; %d bytes)\n",
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Adhokshaj Mishra <me@adhokshajmishraonline.in>");
